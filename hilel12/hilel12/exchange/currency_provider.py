...
class VkurseProvider(ProviderBase):
    ...
    def get_rate(self) -> SellBuy:
        ...
        for currency, data in dict(response.json()).items():
            if currency == currency_from_code and self.currency_to == "UAH":
                value = SellBuy(sell=float(data["sell"]), buy=float(data["buy"]))
                return value
        raise RateNotFound(
...
class NBUStatProvider(ProviderBase):
    ...
    def get_rate(self) -> SellBuy:
        ...
        for currency in response.json():
            if currency["cc"] == self.currency_from and self.currency_to == "UAH":
                value = SellBuy(
