...
def currency_exchange(request):
    ...
    for currency in currencies:
        best_rate = (
            Rate.objects.filter(currency_from=currency).order_by("-sell").first()
        )
...
def exchange_result(request):
    if request.method == "POST":
        ...
        best_rate = (
            Rate.objects.filter(currency_from=from_currency).order_by("-sell").first()
        )
        ...
        if best_rate:
            ...
            return render(
                ...
                {"result": result, "currency": from_currency},
            )

    return render(
        request, "error.html", {"message": "Помилка в підрахунку обміну валюти"}
    )
